...
test("admin providers list and detail", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "已关联模型" })).toBeVisible();
...
test("admin plan review and commission pages render", async ({ page }) => {
  ...
  await expect(page.getByText("关联提供商、弃用和改价在详情")).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "关联提供商" })).toHaveCount(0);
