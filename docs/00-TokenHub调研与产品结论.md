...
现有分销需求已归并为 `docs/15-渠道与分销规则.md`（D39）。本项目后续文档统一继承：

- 渠道主体：平台自营 A、分销商 B、OEM 贴牌 C。A 可建 B/C；B 无下属渠道；C 只建 B。挂在 C 下的 B 共用 C 的积分池、分佣规则和支付商户。
- 一个自然人账号只能归属一个渠道组织。推广角色是代理商与个人推广员（计佣只沿邀请链两跳）。
- 注册即发专属推广码，并记录分佣资格；无资格或未带他人推广码不计佣，分享只发赠送积分。
- 终端用户注册时绑定唯一推广来源；人工改归属必须有平台管理员审计记录。
- 佣金基数是已确认 API 消耗（平台批发价），不是充值金额；所得进佣金钱包，不抵扣 API。
- 佣金先冻结再解冻；退款对所有相关账户成对冲正，不删原流水。
- 第一版不做无限多级分销、自动对外打款、复杂 CRM/合同/发票和多来源拆分归因。
- C 可配独立 Logo 与域名；不维护上游 Provider、密钥、模型目录和网关基建。
...
本节中的矛盾和空白是调研初稿记录。渠道/分销最终以 `docs/15-渠道与分销规则.md` 与 D39 为准。
