...
echo "$channelhtml" | grep -q "创建渠道套餐"
...
echo "$chanhtml" | grep -q "新建渠道"
echo "$chanhtml" | grep -q "渠道租户"
echo "$chanhtml" | grep -q "不能自己添加提供商和模型"
chandetail="$(curl -sf "$WEB_URL/admin/channels/chn_reseller_b")"
echo "$chandetail" | grep -q "从平台目录授权"
echo "$chandetail" | grep -q "保存渠道"
echo "$chandetail" | grep -q "调整额度"
chanconsole="$(curl -sf "$WEB_URL/channel")"
echo "$chanconsole" | grep -q "本渠道模型"
echo "$chanconsole" | grep -q "不能自己添加提供商和模型"
...
echo "$provhtml" | grep -q "列表每行可探测"
echo "$provhtml" | grep -q "租户不能自带"
echo "$provhtml" | grep -q "新建提供商"
echo "$provhtml" | grep -q "已挂模型"
echo "$provhtml" | grep -q "一家提供商可以同时挂多个公开模型"
provdetail="$(curl -sf "$WEB_URL/admin/providers/echo-primary")"
echo "$provdetail" | grep -q "凭据轮换"
echo "$provdetail" | grep -q "账号池"
echo "$provdetail" | grep -q "读取账号"
echo "$provdetail" | grep -q "提供商状态"
echo "$provdetail" | grep -q "编辑"
echo "$provdetail" | grep -q "列表只显示指纹"
...
echo "$modelhtml" | grep -q "租户不能自己添加"
echo "$provdetail" | grep -q "轮换凭据"
