...
- Provider：`GET/POST/PATCH /admin/providers`、`POST /admin/providers/{id}/health-check`（不会计费、不强制确认；管理列表 `/admin/providers` 每行可探测）、`POST /admin/providers/{id}/credentials` 凭据轮换（不回显明文）；
