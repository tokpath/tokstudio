  ...
  return (
    <header ref={rootRef} className="sticky top-0 z-40 border-b border-hairline bg-canvas">
      <div className="mx-auto flex h-16 max-w-[1200px] items-center gap-8 px-6">
        ...
        <nav className="relative hidden flex-1 items-center gap-1 md:flex" aria-label={tc("publicNav")}>
          {MEGA_MENUS.map((menu) => {
            ...
            return (
              <div key={menu.id} className="relative">
                <button
                  ...
                  className={`inline-flex items-center gap-1 rounded-control px-3 py-1.5 text-sm transition-colors duration-150 ${
                    open || active ? "bg-brand-soft text-brand-emphasis" : "text-ink-secondary hover:text-ink"
                  }`}
                >
                  {t(menu.labelKey)}
                  <ChevronDown
                    className={`size-3.5 opacity-60 transition-transform duration-150 ${open ? "rotate-180" : ""}`}
                    strokeWidth={1.75}
                    aria-hidden
                  />
                </button>
          ...
          {openId ? (
            <div
              className="absolute left-0 top-full z-50 mt-2 w-[min(720px,calc(100vw-3rem))] rounded-card border border-hairline bg-canvas-raised p-5 shadow-[0_1px_2px_rgba(20,20,20,0.06)]"
              onMouseLeave={() => setOpenId(null)}
            >
              {MEGA_MENUS.filter((m) => m.id === openId).map((menu) => (
                <div key={menu.id} className="grid gap-8 sm:grid-cols-3">
                            ...
                            <li key={`${col.titleKey}-${link.href}-${link.labelKey || link.literal}`}>
                              ...
                              >
                                ...
                                <span>
                                  ...
                                  {link.hintKey ? <span className="mt-0.5 block text-[12px] leading-relaxed text-ink-mute">{t(link.hintKey)}</span> : null}
                                </span>
                              </Link>
                            </li>
                          );
      ...
      <nav className="flex gap-1.5 overflow-x-auto border-t border-hairline px-6 py-2.5 md:hidden" aria-label={tc("mobileNav")}>
