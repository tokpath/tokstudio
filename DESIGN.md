...
description: TokenHub 的视觉宪法（1.0）。隐喻仍是清算所——公布牌价、扣住预授权、写下不可改的账。公共站的版式手艺参考 Ofox（留白、无框统计、双 CTA、价目作高潮），不搬橙色、狐狸标或促销墙。结构对齐 Stitch DESIGN.md，内容只服务本产品的四个入口、账本和 OEM。导航与信息架构以 docs/13、docs/14 为准，本文件不重开。
colors:
  brand: "#2150D6"
  brand-press: "#183CA8"
  brand-soft: "#DCE6FB"
...
`1.0` 是视觉宪法的锁定版。公共站的版式手艺（大标题、留白、无框统计、双 CTA、价目作高潮）继续有效；下面三条不再写「待真页面压测」，实现必须按此验收：

1. **主题 token** — 四个入口（公共站、用户台、渠道台、管理台）只用语义 CSS 变量（`--canvas` / `--ink` 等），组件禁止写死 `paper-*`。浅色 / 深色 / 跟随系统每个入口都要有。OEM 只换 §2.4 允许的 `theme_json` key，换完必须重测对比度。
2. **账本密度** — 钱包 / 用量 / 佣金 / 路由回单表：金额与 token 用 tabular 数字；宽表 `ScrollTable` 钉首列 + 末列；空状态诚实，不画假图。公共站保持编辑节奏，三个控制台保持账本密度。同一套 token，两套节奏。
3. **高风险盖章确认** — 退款、加款、改佣金、改凭据、改价格底线必须是有重量的第二次盖章，不是轻轻可点掉的对话框。文案对齐 `--danger` / `REVERSED` 语义。

**信息架构不在本文件。** 导航、侧栏条目、总览职责由 `docs/13`（管理台目录与网关 = 提供商 / 模型 / 路由组）和 `docs/14`（用户台总览已瘦身、一页一事）锁死。本文件只规定壳、表、章、确认的视觉；不要在这里重开菜单。
...
**Key Characteristics（1.0 锁定）：**

- Light 是纸，Dark 是碳；四个入口都提供浅色 / 深色 / 跟随系统，组件只写 `--canvas` / `--ink` 等语义变量。
- 品牌只留一枚可替换钴蓝章。颜色表示「这一笔成立了」，不表示「我们是 AI」。OEM 不得改纸/碳或语义色。
- 主角是价目表、路由回单、账本行、佣金冻结，不是摄影或插画。钱与 token 用 `{typography.tabular}`。
- 高风险操作（退款、加款、改佣金、改凭据、改价格底线）必须二次盖章确认，视觉上要像盖章，不像轻点可滑掉的 Dialog。
- 公共站用编辑级节奏（大标题 + 空气 + 数字），控制台用账本级密度。两套节奏共用同一套 token，不换色相。导航条目不在本文件改。
...
- 公共站可以有分层导航（模型 / 文档 / 产品），但不要做成营销海报式 mega menu。当前项用淡章底，不要整段灌钴。
...
实现时映射为 `--canvas` / `--canvas-raised` / `--ink` / `--ink-secondary` / `--ink-mute` / `--hairline`。YAML 里的 `paper-*` / `carbon-*` 只是 Light / Dark **真值表**，给实现映射用，**不是组件 API**。

四个入口的组件、图表、Dialog、空状态一律写语义变量（`bg-canvas`、`text-ink`、`border-hairline` 或 `var(--ink)`）。禁止在 TSX/CSS 里写死 `paper-ink`、`paper-raised`、`#F4F1EA`、`#FFFDF8` 这类纸面色名或 hex。品牌章走 `--brand*`；状态走 `--success` / `--hold` / `--danger` / `--degraded`。蒙层走 `--scrim`（由 `{colors.scrim-light}` / `{colors.scrim-dark}` 映射）。

纸保持微暖，不要改成 Ofox 的冷灰 `#f4f4f4`。暖纸是清算所的触感；冷灰会让四个入口看起来像又一个模型聚合站。

三档：浅色、深色、跟随系统。默认跟随系统，键名 `tokenhub-theme`。四个入口顶栏都要有 `theme-toggle`，不能只做公共站或只做控制台。OEM 可建议默认档，不能关掉其中一档。
...
- OEM 换主色时，浅色和深色都要重测链字/焦点环（正文 ≥ 4.5:1，大标题 ≥ 3:1，实心章上的 `--on-brand` 同样成立）。不达标就拒绝发布该 `theme_json`，不要上线后再补。
- 状态不能只靠颜色：徽章必须带 `READY` / `HOLD` / `REVERSED` 这类字。

### 2.4 `brand.theme_json` 契约

对应 `docs/05` 的 `brand.theme_json`。OEM（渠道 C）**只能**写这些 key（1.0 不增不减；禁止纸/碳/语义色/圆角/字号）。换任何一枚章色后，浅色和深色都要重测对比度，不达标拒绝发布：
...
**侧栏条目与分组不在本文件决定。** 以 `docs/13`（管理台目录与网关 = 提供商 / 模型 / 路由组；价格进模型详情，平台不管用户 Key）和 `docs/14`（用户台总览已瘦身：个人状态 + 用量趋势 + 快捷入口；用量与请求明细拆分）为准。下面只写壳的视觉和门户裁剪纪律；若与那两份冲突，改导航去改那两份，不要在 DESIGN.md 重开 IA。

菜单按门户裁剪（D33），隐藏不是安全边界。视觉验收时按已锁定 IA 对照，不在本表发明新入口：

| 入口 | 壳怎么长（条目以 docs/13、docs/14 为准） |
| --- | --- |
| 用户台 | 总览已瘦身；其余一页一事。详见 `docs/14` |
| 渠道台 | 本渠道用户、套餐、推广、额度、用量、佣金/结算 |
| 管理台 | 目录与网关只有提供商 / 模型 / 路由组。详见 `docs/13` |
...
**账本密度（1.0 锁定）。** 钱包流水、用量汇总、佣金/结算、路由回单（请求明细）必须是账本，不是仪表盘海报：

- 金额、token、单价、汇率用 `{typography.tabular}`（`tnum` / `tabular-nums`），右对齐。ID 用 `{typography.mono}`。不要把钱写成正文比例数字。
- 宽表用 `ScrollTable`（或 `AdminListPanel`）：外层横向滚动；**默认钉住第一列与最后一列**，中间列滑动。列数少于 3 时不钉。钉列用抬起表面（`--canvas-raised`）+ 轻阴影，悬停与行同步。
- 空表用 `empty-state` 说人话（「还没有用量事件」「还没有冻结佣金」）。不要骨架闪烁假装有数，不要画一条假的平滑曲线或空饼图占位。
- 控制台表区行高走 `ledger-row`（约 12×16 内边距）。公共站价目表可以松一档（16–20px），让单价先被看见。同一套 token，两套节奏：公共站编辑级，控制台账本级。

表头用 `{typography.eyebrow}`。行 1px 底边。悬停：淡章 40%，不要投影。选中：`--brand-soft`。

分页、筛选是次按钮，不是主章。

新建 / 编辑：字段少（约 ≤5）用 Dialog；多区块、凭据、生命周期用独立详情页。不要把长表单堆在列表页底部。短表单 Dialog 与宽表钉列的页面职责见 `docs/13` / `docs/14`，本文件不重开。

### 4.6 登录与高风险确认

**`auth-card`**：居中抬起卡片。邮箱密码、Google。推广码不展示为可改归属；注册后渠道不可自助切换。没有渐变英雄，没有社交插画。登录页可以左右分栏（左一句承诺 + 右表单），但左栏仍然是纸上的字，不是摄影。

**`confirm-dialog`（1.0 锁定：盖章，不是轻 Dialog）。** 退款、手工加款、佣金调整、凭据修改、价格底线必须走这层。视觉要对齐「签核章」：抬起卡片 + 蒙层 + 说清后果的标题 + 一枚实心确认章。不要做成 toast、不要做成行内轻提示、不要做成点蒙层就当确认的薄层。

- **必须二次盖章的动词**：退款 / 冲正、手工加款（top-up）、改佣金、改上游凭据、改价格底线。缺确认的请求应被后端拒绝（现有 `X-Tokenhub-Confirm`），前端不能用「知道了」代替。
- **标题写后果，不写动词复述。** 对齐危险语义：会冲正佣金用 `REVERSED` / 冲正；会停止新请求写清楚；退款成功留在页内回单，不用自动消失的 toast 报喜。
- **主操作**：危险场景用 `button-danger`（朱红实心章），普通确认用 `button-primary`。取消是 `button-secondary` 或 ghost，不是对称的两枚主章。
- **重量**：层级 3（§6）：蒙层 `{colors.scrim-light}` / `{colors.scrim-dark}`（实现为 `--scrim`），卡片用抬起表面，不要轻阴影浮层。点蒙层或 Esc 只能取消，不能确认。主操作必须再点一次实心章。
- **不要**：可滑掉的 sheet、右上角叉当作唯一出口、默认焦点落在确认章上、用 `hold` 色的警告图标代替后果文案。`--hold` 表示扣住/待对账，冲正/退款用 `--danger`。

### 4.7 空状态、Toast、徽章

**`empty-state`**：一句人话 + 一个次按钮。例如「还没有用量事件」「还没有冻结佣金」。不要机器人插画，不要用假图表或骨架行把空账本装成「已有数据」。
...
### 4.9 图表（ECharts，用量与总览）

用户台 / 渠道台用量、管理台总览与用量回放都用图：先看见趋势和分布，表格用来对账。

管理台总览要能看：成功率、P50/P95、上游错误、平台成本、客户收入、毛利、余额风险、佣金负债、渠道消耗、待对账。

- 轴和标注：`--ink-mute`，数字 tabular。
- 网格：`--hairline`，不要渐变面积。
- 主序列：`--brand`；对比序列：`--ink-secondary`。请求与金额用双 Y 轴，避免量纲把其中一条压扁。
- 账本类堆叠：入账 `--success`，占用 `--hold`，冲正 `--danger`。毛利不要用彩虹。
- Tooltip：`chart-tooltip`，纸/碳抬起 + 细线。
- 无数据：`empty-state`，不要画一条假的平滑曲线，也不要用「示例趋势」冒充本账本。有真实事件才画图。
...
- 四个入口都提供浅色 / 深色 / 系统；组件只写 `--canvas` / `--ink` 等语义变量。
- 把路由回单、价目、账本行当主角。钱和 token 用 tabular，宽表钉首末列。
- 公共站用大标题、留白、无框统计、双 CTA；控制台保持账本密度。同一套 token，两套节奏。
- 金额等宽，ID 等宽，状态写字。空账本说人话，不画假图。
- 高风险操作走盖章确认（`confirm-dialog`）并写审计；危险主操作对齐 `--danger` / `REVERSED`。
- OEM 只换 `theme_json` 允许的章和 Logo，换完重测对比度。
- 敏感字段按门户藏：前端藏了，后端仍要拒。
- 导航以 `docs/13` / `docs/14` 为准，本文件不重开。
...
- 不要把控制台锁深色、公共站锁浅色。
- 不要用 `dark:` 写另一套布局，也不要在组件里写死 `paper-*` / 纸面 hex。
- 不要胶囊 CTA、大光晕、机器人空状态。
- 不要一个数字同时表示收入、成本和佣金。
- 不要用可滑掉的轻 Dialog 或 toast 做退款 / 加款 / 改佣金 / 改凭据 / 改底价。
- 不要在文档或示例里写真实完整 Key。
- 不要为「好看」展示平台成本或他人渠道数据。
- 不要在本文件重开 `docs/13` / `docs/14` 已锁定的导航。
...
7. 改品牌色时浅色和深色都验对比度。
8. 高风险动词必须进有重量的 `confirm-dialog`（盖章，不是轻 Dialog）。
9. 不要改导航；条目以 `docs/13` / `docs/14` 为准。
10. 不要从 Ofox 或 awesome-design-md 某站整页抄皮肤；只许借用本文件已写进「能做」的原则。

可用短指令：

> 按 DESIGN.md 的 TokenHub Clearing 1.0：纸/碳双主题，组件只写语义 CSS 变量，一枚钴蓝章。公共站用 56px 标题、无框统计、双 CTA、curl 和路由回单对页、价目作高潮。控制台账本密度：tabular 数字，宽表钉首末列，空状态不画假图。高风险走盖章确认。圆角 10/12/8。不要橙，不要胶囊，不要生成图墙。不要重开导航。
...
- 主题 token：四入口语义变量、浅/深/系统、OEM 只换 §2.4 允许的 `theme_json`，换完重测对比度。语义色与纸/碳禁止 OEM 改。
- 账本密度：钱包 / 用量 / 佣金 / 路由回单用 tabular；宽表 `ScrollTable` 钉首末列；空状态诚实。公共站编辑节奏，控制台账本密度。
- 高风险盖章确认：退款、加款、改佣金、改凭据、改价格底线必须是有重量的第二次确认，文案对齐 `--danger` / `REVERSED`。
- 信息架构指针：导航由 `docs/13`、`docs/14` 所有，本文件不重开。

仍属实现细节（不挡 1.0，也不得回头改上面三条）：

- 图表色在色觉障碍下的成对组合，用真实数列复验，不改 token 名或语义色 hex。
- 具体 Logo 路径图形（折线 vs 钴方）可在品牌资源里二选一，不得改成发光标或动物标。
