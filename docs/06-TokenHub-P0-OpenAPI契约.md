...
- 渠道额度：`GET /channel/quota`、`GET /channel/allocations`、`GET /admin/channel-quotas/{channel_id}`、`POST /admin/channel-quotas/grant`；`quota` 含 `issued_minor`/`consumed_minor`/`allocation_count`；B/C 兑换或确认入账时按 1:1 发放，渠道额度不足返回 `402 insufficient_quota`；管理页 `/admin/channels` 可读取并调整额度；渠道页「已发放额度」列出下属发放；
