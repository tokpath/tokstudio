    ...
    <section className="rounded-card border border-hairline bg-canvas-raised p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-lg font-semibold">{t("try", { name: displayName })}</h2>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button asChild type="button" variant="outline">
              <Link href="/login">{t("loginTry")}</Link>
            </Button>
            <span className="self-center text-[12px] text-ink-mute">{t("kind", { kind })}</span>
          </div>
        </div>
      ) : (
        <div className="mt-4 flex flex-col gap-3">
