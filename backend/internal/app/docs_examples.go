...
func docsExamples(apiDomain, model string) gin.H {
	...
	return gin.H{
		"curl":     "curl -sS " + base + "/v1/chat/completions -H 'Authorization: Bearer $TOKENHUB_API_KEY' -H 'Content-Type: application/json' -d '" + chatBody + "'",
		...
		"messages": "curl -sS " + base + "/v1/messages -H 'Authorization: Bearer $TOKENHUB_API_KEY' -H 'Content-Type: application/json' -d '{\"model\":\"" + model + "\",\"max_tokens\":32,\"messages\":[{\"role\":\"user\",\"content\":\"hi\"}]}'",
		"video":    "curl -sS " + base + "/v1/videos -H 'Authorization: Bearer $TOKENHUB_API_KEY' -H 'Content-Type: application/json' -d '{\"model\":\"tokenhub/video-demo\",\"prompt\":\"a river at dusk\"}'",
	}
}

func docsNotes() gin.H {
	return gin.H{
		"auth":       "用 Authorization: Bearer $TOKENHUB_API_KEY。示例里不会写入完整 Key，请到控制台复制自己的 Key。",
