...
test("admin providers list and detail", async ({ page }) => {
  ...
  await expect(page.getByRole("columnheader", { name: "已挂模型" })).toHaveCount(0);
  ...
  await expect(page.getByRole("heading", { name: "已挂模型" })).toBeVisible();
  await expect(page.getByText("把这家上游的 API Key 换成一把新的")).toBeVisible();
...
test("admin plan review and commission pages render", async ({ page }) => {
  ...
  await expect(page.getByText("挂载、弃用和改价在详情")).toBeVisible();
  await expect(page.getByRole("columnheader", { name: "公开 ID" }).first()).toBeVisible();
  await expect(page.getByRole("columnheader", { name: "途径" }).first()).toBeVisible();
  await expect(page.getByRole("heading", { name: "同步上游" })).toHaveCount(0);
  await expect(page.getByRole("heading", { name: "挂载 Provider" })).toHaveCount(0);
